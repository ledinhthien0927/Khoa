...
public class IcePotionProjectile : MonoBehaviour
{
    [Header("Arc Settings")]
    ...
    public void SetTarget(Transform newTarget, float duration, Vector3 spawnPos)
    {
        target = newTarget;
    ...
    private void Update()
    {
        ...
        if (target == null && followMovingTarget)
        {
            Destroy(gameObject);
            return;
        }
    ...
    private void Explode(Vector3 impactPos)
    {
        ...
        for (int i = 0; i < hits.Length; i++)
        {
            ...
            if (player != null)
            {
                FreezeController freezer = player.GetComponent<FreezeController>();
                if (freezer == null)
                {
                    freezer = player.gameObject.AddComponent<FreezeController>();
                }

                freezer.Freeze(freezeDuration);
            }
        }
